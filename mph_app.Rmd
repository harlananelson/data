...
filter_data_by_measure <- function(d, measure_list){
  d  %>% 
    dplyr::filter(measure %in% measure_list | 'All' %in% measure_list)  
}
```
...
```{r}
# Update the tag choices after selecting a measure or topic
...
```{r}
# Update the measure choices after selecting a tag or topic
...
```{r}
# Update the topic choices after selecting a measure or tag
observeEvent(c(input$measure, input$tag, input$topic), {  
...
checkboxGroupInput("measure", label = h3("Topic Selection"),  
                   choices = choices_list_topic, 
